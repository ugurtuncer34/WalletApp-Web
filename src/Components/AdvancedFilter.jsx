...
export default function AdvancedFilter({ filters, masterData, categories, onEdit, onDelete }) {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-4 md:p-5 rounded-3xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-gray-100 dark:border-gray-700 transition-colors">
            <h3 className="text-gray-800 dark:text-gray-100 font-bold mb-4 flex items-center gap-2">🔍 Gelişmiş Harcama Arama</h3>
            <form onSubmit={handleFilterSearch} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 items-end">
                ...
                <div className="flex gap-2">
                    ...
                    {hasSearched && <button type="button" onClick={handleClearFilter} className="px-4 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 font-bold text-sm py-3 rounded-xl transition">✕</button>}
                </div>
            </form>

            {hasSearched && (
                <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-700 animate-fadeIn">
                    <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-4 uppercase tracking-wider">Sorgu Sonuçları ({filteredTransactions.length} Eşleşme)</h4>
                    <div className="max-h-[350px] overflow-y-auto space-y-2 pr-2">
                        {filteredTransactions.map((t) => {
                            ...
                            return (
                                <div key={t.id} className="p-3 bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100/70 dark:hover:bg-gray-700 rounded-xl flex items-center justify-between border border-gray-100/50 dark:border-gray-600 transition duration-200 group">
                                    <div className="flex items-center gap-3">
                                        <span className="text-xl bg-white dark:bg-gray-800 w-9 h-9 rounded-full flex items-center justify-center shadow-sm">{t.categoryIcon}</span>
                                        <div>
                                            <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm capitalize">{(t.description || t.categoryName || "").toLocaleLowerCase('tr-TR')}</p>
                                            <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wide">
                                                {formatDateTime(t.date)} • <span className="text-gray-500 dark:text-gray-400">{subtitleCategory}</span> {t.merchantName ? `• ${t.merchantName}` : ''} {t.countryName && t.countryName !== 'Türkiye' ? `• ${t.countryName}` : ''}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-end w-[160px] gap-2">
                                        <div className="font-bold text-gray-800 dark:text-gray-100 text-sm text-right flex-1">
                                        ...
                                        <div className="w-[60px] flex items-center justify-end gap-1">
                                            <button onClick={() => onEdit(t)} className="text-gray-400 dark:text-gray-300 hover:text-blue-600 bg-transparent hover:bg-blue-50 dark:hover:bg-gray-600 p-1.5 rounded-lg transition">✏️</button>
                                            <button onClick={() => onDelete(t.id)} className="text-gray-400 dark:text-gray-300 hover:text-red-600 bg-transparent hover:bg-red-50 dark:hover:bg-gray-600 p-1.5 rounded-lg transition">🗑️</button>
                                        </div>
                                    </div>
                                </div>
                            );

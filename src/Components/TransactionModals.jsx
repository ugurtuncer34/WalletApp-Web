...
export function QuickAddModal({ isOpen, onClose, chipName, onSubmit, loading }) {
    ...
    return (
        // 1. KAPSAYICI: Sadece tıklamaları açıp kapatır, animasyon barındırmaz.
        <div className={`fixed inset-0 z-[70] flex items-center justify-center p-4 ${
            isOpen ? "pointer-events-auto" : "pointer-events-none"
        }`}>
            
            {/* 2. ARKA PLAN (BACKDROP): Kendi opacity animasyonu var, şak diye kapanmaz! */}
            <div 
                className={`absolute inset-0 bg-gray-900/40 dark:bg-black/60 backdrop-blur-sm transition-opacity duration-300 ease-out ${
                    isOpen ? "opacity-100" : "opacity-0"
                }`} 
                onClick={onClose}
            ></div>

            {/* 3. MODAL KUTUSU: Kendi Scale & Translate animasyonu var */}
            <div className={`relative bg-white dark:bg-gray-800 rounded-3xl p-6 w-full max-w-sm shadow-2xl border border-gray-100 dark:border-gray-700 transform transition-all duration-300 ease-out ${
...
export function EditModal({ isOpen, onClose, initialData, masterData, onSubmit, loading }) {
    ...
    return (
        <div className={`fixed inset-0 z-[70] flex items-center justify-center p-4 ${
            ...
            {/* MODAL KUTUSU */}
            <div className={`relative bg-white dark:bg-gray-800 rounded-3xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto border border-gray-100 dark:border-gray-700 transform transition-all duration-300 ease-out ${
                isOpen ? "scale-100 translate-y-0 opacity-100" : "scale-95 translate-y-4 opacity-0"
            }`}>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Tutar *</label>
                        <input type="number" step="0.01" required value={editForm.amount || ""} onChange={e => setEditForm({ ...editForm, amount: e.target.value })} className="p-3 bg-gray-50 dark:bg-gray-900 text-sm text-gray-700 dark:text-gray-200 rounded-xl border border-transparent dark:border-gray-700 focus:outline-none focus:bg-white dark:focus:bg-gray-950 focus:border-blue-500 transition" />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Kategori *</label>
                        <CategorySelect options={groupedCategories} value={editForm.categoryId || ""} onChange={(val) => setEditForm({ ...editForm, categoryId: val })} placeholder="Seçiniz..." />
                    </div>
                    <div className="flex flex-col gap-1 md:col-span-2">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Açıklama</label>
                        <input type="text" value={editForm.description || ""} onChange={e => setEditForm({ ...editForm, description: e.target.value })} className="p-3 bg-gray-50 dark:bg-gray-900 text-sm text-gray-700 dark:text-gray-200 rounded-xl border border-transparent dark:border-gray-700 focus:outline-none focus:bg-white dark:focus:bg-gray-950 focus:border-blue-500 transition" />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">İşyeri</label>
                        <SearchableSelect options={getFilteredMerchants(editForm.categoryId)} value={editForm.merchantId || ""} onChange={(val) => setEditForm({ ...editForm, merchantId: val })} placeholder="İsteğe Bağlı" emptyLabel="Boş Bırak (Temizle)" />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Tarih</label>
                        <input type="datetime-local" value={editForm.date || ""} onChange={e => setEditForm({ ...editForm, date: e.target.value })} className="p-3 bg-gray-50 dark:bg-gray-900 text-sm text-gray-700 dark:text-gray-200 rounded-xl border border-transparent dark:border-gray-700 focus:outline-none focus:bg-white dark:focus:bg-gray-950 focus:border-blue-500 transition" />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Ülke</label>
                        <select value={editForm.countryId || ""} onChange={e => setEditForm({ ...editForm, countryId: e.target.value })} className="p-3 bg-gray-50 dark:bg-gray-900 text-sm text-gray-700 dark:text-gray-200 rounded-xl border border-transparent dark:border-gray-700 focus:outline-none focus:bg-white dark:focus:bg-gray-950 focus:border-blue-500 transition">

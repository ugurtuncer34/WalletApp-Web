...
This project is built as a React-based SPA (Single Page Application) that communicates with a robust .NET Core backend gateway, which securely orchestrates AI tasks with an internal Python NLP microservice.
...
* **🤖 AI-Powered Bulk Import:** Seamlessly upload bank statement PDFs. The frontend securely transmits the file to the .NET backend gateway, which leverages an internal Python NLP microservice (powered by DeepSeek) to parse the document, resolve entities via fuzzy matching, and return a smart review grid before bulk inserting into the database. This feature is fully toggleable via environment variables.
...
* **PWA:** vite-plugin-pwa
* **Integration:** RESTful APIs (.NET Core Backend acting as a secure Gateway/BFF)
...
2. **Configure Environment Variables:**
   Create an `.env` or `.env.local` file in the root directory and configure your backend URL and feature toggles:
   ```env
   # .NET Core Backend API Gateway URL
